        ...
        >
            <motion.div className="absolute inset-0 z-10" animate={{ opacity: isHovered ? 0 : 1 }}>
                <img
                    ...
                    // Borrada la línea del @ts-expect-error porque TS ya es inteligente
                    fetchPriority={index === 0 ? "high" : "auto"}
                    loading={index < 2 ? "eager" : "lazy"}
                    width={isFeatured ? 800 : 450}
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 text-left bg-gradient-to-t from-black via-black/90 to-transparent">
                    <p className="font-mono text-[8px] text-[#cdc69c]/70 mb-1 tracking-[0.2em] uppercase">
                        {isFeatured ? "PREMIUM_COLLECTION" : `RD-202${index}`}
                    </p>
                    <h2 className={`font-black tracking-tighter text-white uppercase leading-[0.9] 
                        ${isFeatured ? "text-4xl md:text-7xl" : "text-xl md:text-2xl"}`}>
                        {project.title}
                    </h2>
                </div>
            </motion.div>

            <motion.div className="absolute inset-0 z-20 flex flex-col justify-between p-5 overflow-hidden"
            ...
            >
                {!prefersReducedMotion && isHovered && project.video && (
                    <video
                        ...
                        className="absolute inset-0 z-0 object-cover w-full h-full opacity-30 grayscale mix-blend-overlay"
                    />
                )}
                ...
                <div className="relative z-10 flex flex-col h-full text-left">
                    ...
                    <p className={`font-mono leading-tight mb-4 italic ${isFeatured ? "text-base md:text-xl" : "text-[11px] md:text-sm"}`} style={{ color: theme.text }}>
                        "{project.desc}"
                    </p>
                    ...
                    <div className="p-3 mt-auto border-2 border-current border-dashed bg-black/5" style={{ color: theme.text }}>
                        ...
                        <div className="flex flex-wrap gap-1">
                            {project.techStack?.split(',').map((tech, i) => (
                                <span key={i} className="font-mono bg-black/10 px-1.5 py-0.5 font-bold text-[8px] md:text-[9px]">
                                    {tech.trim()}
                                </span>
                            ))}
                        </div>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={e => e.stopPropagation()}
                className="bg-[#f5f3e7] max-w-5xl w-full flex flex-col md:flex-row overflow-hidden shadow-[20px_20px_0px_#8e2b27] border-2 border-black relative">
                <div className="relative w-full border-b-4 border-black h-60 md:w-3/5 md:h-auto md:border-b-0 md:border-r-4 bg-black">
                    <img src={project.img} className="object-cover w-full h-full" alt={project.title} loading="eager" />
                </div>
                <div className="relative flex flex-col justify-between p-6 md:p-10 md:w-2/5">
                    <button onClick={onClose} className="absolute text-black top-4 right-4 hover:rotate-90 transition-transform" aria-label="Cerrar modal">
                        <FaTimes size={24} />
                    </button>
                    <div className="space-y-6">
                        <div>
                            <span className="text-[9px] font-mono text-[#8e2b27] font-black tracking-widest uppercase block mb-1">// DATA_LOG_01</span>
                            <h2 className="text-4xl font-black text-black uppercase tracking-tighter">{project.title}</h2>
                        </div>
                        <p className="font-mono text-xs leading-relaxed text-black/80 border-l-4 border-[#8e2b27] pl-4">{project.desc}</p>
                    </div>
                    <a href={project.url} target="_blank" rel="noreferrer"
                        className="mt-8 bg-[#8e2b27] text-white py-4 text-center text-[10px] font-black tracking-[0.3em] uppercase hover:bg-black transition-colors">
...
export default function Proyectos() {
    ...
    return (
        <main className="relative min-h-screen" style={{ backgroundColor: COLORS.bg }}>
            ...
            <section className="relative z-999 px-6 pt-24 pb-12 mx-auto text-center max-w-7xl">
                <span className="absolute top-6 left-1/2 -translate-x-1/2 text-[18vw] font-black text-white/20 select-none pointer-events-none uppercase leading-none">
                    Catalog
                </span>
                <h1 className="flex flex-col items-center leading-[0.8] mb-6 relative z-10">
                    <span className="text-5xl md:text-[9rem] font-black uppercase tracking-tighter text-black">PROJECT</span>
                    <div className="flex items-center gap-3 mt-2 md:gap-8">
                        ...
                        <span className="text-4xl md:text-[9rem] font-black uppercase italic text-black"
                            style={{ WebkitTextStroke: `1.5px ${COLORS.accent}`, paintOrder: 'stroke fill' }}>
                            PRESS
                        </span>
            ...
            <section className="px-6 pb-32 mx-auto max-w-7xl relative z-10">
                ...
                <div className="flex flex-col items-center mt-24">
                    <Link to="/Feedback" className="flex items-center gap-4 group w-fit" aria-label="Contacto">
                        <motion.div whileHover={{ x: 10 }} className="flex items-center gap-4 cursor-pointer">
                            ...
                            <span className="text-xl md:text-3xl font-black uppercase tracking-tighter text-[#8e2b27] group-hover:text-black">
